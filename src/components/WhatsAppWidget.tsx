import { useState } from "react";
import { MessageCircle, X } from "lucide-react";

const WA_URL = "https://wa.me/2349090336443";

export function WhatsAppWidget() {
  const [open, setOpen] = useState(false);
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
      {open && (
        <div className="rounded-2xl bg-white shadow-2xl border border-border p-4 max-w-xs animate-fade-in">
          <div className="flex justify-between items-start gap-3">
            <div>
              <p className="font-display text-lg text-primary">Need help?</p>
              <p className="text-sm text-muted-foreground mt-1">Chat with our team on WhatsApp. We reply within minutes.</p>
            </div>
            <button onClick={() => setOpen(false)} aria-label="Close" className="text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
          </div>
          <a href={WA_URL} target="_blank" rel="noopener noreferrer"
             className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-[oklch(0.65_0.18_150)] hover:bg-[oklch(0.58_0.18_150)] text-white px-4 py-2 text-sm font-semibold transition">
            Start Chat
          </a>
        </div>
      )}
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Open WhatsApp chat"
        className="pulse-ring h-14 w-14 rounded-full bg-[oklch(0.65_0.18_150)] hover:bg-[oklch(0.58_0.18_150)] text-white shadow-xl flex items-center justify-center transition"
      >
        <MessageCircle className="h-7 w-7" />
      </button>
    </div>
  );
}
